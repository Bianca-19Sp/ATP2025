#TPC6 - Aplicação de Meteorologia

def criarTabMeteo(t):
    ndias = int(input("Inseres o número de dias que pretendes registar:"))
    while ndias > 0:
        dia = []
        ano = int(input("Insere o ano do registo:"))
        mes = int(input("Insere o mês do registo[na forma numérica]:"))
...
def guard(t):
    ...
    for linha in t:
            ...
            file.write(linha)
    file.close()
    print("O seu ficheiro foi guardado!")
    return
...
(1) Calcular média das temperaturas máxima e mínima;
...
(10) Sair""")
        ...
        if op == 0:
            ...
            if t == []:
                print("Não existem dados para executar esta tarefa...")
            else:
                print("A média das temperaturas máxima e mínima é:", medias(t))
                medias(t)
        elif op == 2:
            if t == []:
